...
class SetUpAccountPage:
    ...
    def __init__(self, page):
        ...
        self.registration_number_input = self.page.locator("myt-input", has_text="HCP registration number").locator(
            "input")
        self.complete_setup_btn = self.page.get_by_role("button", name="Complete setup")

        # I am involved in running clinical trials *
        self.is_running_clinical_trials_yes_btn = self.page.locator("mat-radio-button", has_text="Yes")
        self.is_running_clinical_trials_no_btn = self.page.locator("mat-radio-button", has_text="No")

    @allure.step("Setup account")
    def setup_account(self, job_title: str, specialty: str, country: str, registration_number: str,
        ...
        with allure.step(f"Select if user is running clinical trials: {is_running_clinical_trials}"):
